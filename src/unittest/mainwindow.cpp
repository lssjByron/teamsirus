...
///*
// * When a user inputs a CSV file, there a four kinds they can input; Teaching, Publications, Presentations, and Funding.
// * The original method checked for these types of CSV's by simply looking for a particular "searchstring" for each of
// * the four categories within the header of the in-file.
// *
// * This caused an error in which a CSV that contained all empty cells other than the search string in it would be allowed
// * to come into the program to be read and displayed to the user
// */
...
//        /*
//         * Here we let the first if go through normally --
//         * We do this since the old boolean value inside was:
//         *      teachPath.isEmpty() || (!teachPath.isEmpty() && filePath.compare(teachPath))
//         * which simply checks if the current infile is new or replacing another
//         */
...
//            // This is where the error happens. Simply checking for one single unique header is not enough
...
//         * Here we let the first if go through normally --
//         * We do this since the old boolean value inside was:
//         *      presPath.isEmpty() || (!presPath.isEmpty() && filePath.compare(presPath))
//         * which simply checks if the current infile is new or replacing another
//         */
//        // read if first or if a new file is loaded
//        if (true) {
...
//            /* We don't need this either for the error
...
//         * Here we let the first if go through normally --
//         * We do this since the old boolean value inside was:
//         *      fundPath.isEmpty() || (!fundPath.isEmpty() && filePath.compare(fundPath))
//         * which simply checks if the current infile is new or replacing another
//         */
//        // read if first or if a new file is loaded
//        if (true) {
...
///*
// * With this new function; we are checking the infiles header with the another vector that houses the correspoinding FULL header.
// * This full header has been obtained from the CSV's that have been supplied.
// *
// * By checking the infile's header fully to see if it matches the corresponding csv type (Teaching, Presentation, Publication, Funding)
// * we can be sure that any infile thats dont match are rejected.
// */
////int MainWindow::checkFileFixed(int index, QString filePath) {
////    CSVReader reader;
////    std::vector<std::string> header;
////    std::string searchstring;
...
////    // A set of vectors that house the headers of each corresponding CSV file types
////    // These will be used to check the validity of the input files
...
////            // This is where the error used to happen. It has now been fixed with the following:
////            // To check if the input file is valid:
////            //      simply check for equality between the input header and the header to the corresponding CSV type

////            if (header == teachHeaderCheck) {
...
////            if (header == publicationsHeaderCheck) {
////                // load in data into the manager, with the date as the key
////                sortHeaderIndex = pubdb->getHeaderIndex("Status Date");
////                pubData = reader.getData();
////                std::vector<std::vector<std::string>*> f_errs;
...
////            if (header == presentationsHeaderCheck) {
...
//    //Note: this is horrendous, don't push blank data into alldata to start, fix this in CSVReader.cpp
